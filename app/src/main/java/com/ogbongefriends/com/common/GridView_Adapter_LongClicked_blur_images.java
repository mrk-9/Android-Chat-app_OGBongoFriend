...
public abstract class GridView_Adapter_LongClicked_blur_images extends BaseAdapter{
 ...
 int [] imageId;
 private ImageLoaderBlur imageLoader_blur;
 ...
 private DisplayImageOptions options;
    ...
    public GridView_Adapter_LongClicked_blur_images(Context mainActivity, ArrayList<HashMap<String, String>> data) {
        ...
        detailPageListener = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				onItemClick(v, String.valueOf(v.getTag()),Integer.parseInt(String.valueOf(v.getTag(R.id.about_myself))));
			}
		};
...
//			
         ...
         imageLoader_blur=new ImageLoaderBlur(context);
    }
 ...
 if(row.get(DB.Table.user_master.last_seen.toString()).length()>0){
 	 if((Long.parseLong(row.get(DB.Table.user_master.last_seen.toString()))/1000)<(5*60)){
 ...
 if(Integer.parseInt(row.get("pass_status"))==0){
	 rowView.setTag(R.id.about_myself, 0);
	 imageLoader_blur.DisplayImage(path, holder.img);
	 holder.tv.setText(CelUtils.shuffleString(row.get("user_name")));
	 
 }
 
 else{
	 holder.tv.setText(row.get("user_name"));
	 rowView.setTag(R.id.about_myself, 1);
 if(path.length()>3){
...
if(UserRating==5){
 ...
 rowView.setOnClickListener(detailPageListener);
    ...
    protected abstract void onItemClick(View v, String string,int pass_status);
