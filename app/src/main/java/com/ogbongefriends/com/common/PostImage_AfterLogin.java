...
public abstract class PostImage_AfterLogin extends AsyncTask<String, Void, String> {
	...
	// ******* DECLARING VARIABLES *******
		...
		public PostImage_AfterLogin(File fl, String url, String filename, String userId/*, long feedId*/, String type, String upload_type, Context c) {
	pref=new Preferences(c);
		...
		protected String doInBackground(String... params) {
			...
			//	fis=Utils.ResizeAndCompressImage(f,60);

				//HttpFileUploader htfu = new HttpFileUploader(Url, Media.IMAGE_DIR, FileName, UserId/*, FeedId*/, Type, Upload_type);
				HttpFileUploader htfu = new HttpFileUploader(Url, pref.get(Constants.KeyUUID), FileName, UserId/*, FeedId*/, Type, pref.get(Constants.kAuthT),Upload_type);
				htfu.doStart(fis);
			...
			if (res != null) {
				...
				try {
					...
					if (obj == null) {
						...
						imageName=objJson.get("file_name").getAsString();
