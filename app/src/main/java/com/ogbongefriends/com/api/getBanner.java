...
public class getBanner extends BasicApi implements Runnable {
...
//	private SharedPreferences pref;
	...
	public getBanner(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		// url = "http://tmwtg.com/tmwtg/Cpanels/signup";
		url = Utils.getCompleteApiUrl(ctx, R.string.GetBanner);
				...
				if (objJson.get("Data") != null) {
					...
					try {
						...
						for(int i=0;i<userdata.size();i++){
							...
							if (singleCountry.has("image")){
								HashMap<String, String>data_to_insert= CelUtils.getMapFromJsonObject(singleCountry);
								db.autoInsertUpdate(DB.Table.Name.banner_master, data_to_insert, DB.Table.banner_master.id+" = "+"'"+ singleCountry.get("id").getAsString() +"'", null);
							}
							}
				db.close();
					}
