...
public class Common_Pages extends BasicApi implements Runnable {
	...
	public String postData,page_name,page_content;
	...
	public Common_Pages(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		//setPostData(map);
		url = Utils.getCompleteApiUrl(ctx, R.string.otherInfo);
		...
		try {
			...
			if (obj == null) {
				...
				if (objJson.has("Data")) {
					...
					page_name=CountryJson.get("page_name").getAsString();
					page_content=CountryJson.get("page_content").getAsString();
