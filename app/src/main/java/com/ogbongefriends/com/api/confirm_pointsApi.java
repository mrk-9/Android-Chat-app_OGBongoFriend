...
public class confirm_pointsApi extends BasicApi implements Runnable {
	...
	public confirm_pointsApi(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		//setPostData(map);
		url = Utils.getCompleteApiUrl(ctx, R.string.points_info);
