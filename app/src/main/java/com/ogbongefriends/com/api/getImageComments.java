...
public class getImageComments extends BasicApi implements Runnable {
...
//	private SharedPreferences pref;
	...
	public getImageComments(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
				...
				if (objJson.get("Data") != null) {
					...
					try {
						...
						for(int i=0;i<userdata.size();i++){
							...
							db.autoInsertUpdate(Table.Name.event_master, data_to_event, Table.event_master.id+" = "+"'"+ singleCountry.get("id").getAsString() +"'", null);							
						}
				db.close();
