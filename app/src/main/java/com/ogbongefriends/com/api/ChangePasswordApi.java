...
public class ChangePasswordApi extends BasicApi implements Runnable{
	...
	public ChangePasswordApi(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		//setPostData(map);
		url = Utils.getCompleteApiUrl(ctx, R.string.change_password_api);
...
public void setPostData(HashMap<String, String> map){
	...
	private String getPostData(HashMap<String, String> map12) {
		...
		try {
			...
			json2.put("old_password",map12.get("old_password"));
			json2.put("new_password", map12.get("new_password"));	
