...
public class getAlbumApi extends BasicApi implements Runnable {
...
//	private SharedPreferences pref;
 ...
 public	JsonArray photos_of_you=null;
 public	JsonArray backstage_photos=null;
	CustomLoader p;
	Preferences pref;

	public getAlbumApi(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		// url = "http://tmwtg.com/tmwtg/Cpanels/signup";
		url = Utils.getCompleteApiUrl(ctx, R.string.get_album);
		// postData = getPostData();
		Log.d("URL ", ""+url);
		pref = new Preferences(ctx);
	}
	
	public getAlbumApi(Context ctx, DB db,String page , CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
	...
	protected void onResponseReceived(InputStream is) {
		...
		try {
			...
			if (obj == null) {
				...
				JsonObject albumData = obj.get(Constants.kappTag).getAsJsonObject();
				resCode = objJson.get("res_code").getAsInt();
				resMsg = objJson.get("res_msg").getAsString();
				
				
				
				if (objJson.get("albumData") != null) {

					
					albumData = objJson.get("albumData").getAsJsonObject();
					photos_of_you=albumData.get("photos_of_you").getAsJsonArray();
					backstage_photos=albumData.get("backstage_photos").getAsJsonArray();
