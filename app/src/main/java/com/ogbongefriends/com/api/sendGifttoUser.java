...
public class sendGifttoUser extends BasicApi implements Runnable {
...
//	private SharedPreferences pref;
 ...
 public	JsonArray userdata=null;
	...
	public sendGifttoUser(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		
		// gift_master_d=-1 in case of send points as gift
		// url = "http://tmwtg.com/tmwtg/Cpanels/signup";
		url = Utils.getCompleteApiUrl(ctx, R.string.sendGift);
		// postData = getPostData();
		Log.d("URL ", ""+url);
		pref = new Preferences(ctx);
	}
	
	public sendGifttoUser(Context ctx, DB db,String page , CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
	...
	private String getPostData(HashMap<String, String> map12) {
		...
		try {
			...
			json2.put("gift_master_id", map12.get("gift_master_id"));
			json2.put("description", map12.get("description"));
			json2.put("other_uuid", map12.get("other_uuid"));
			json2.put("send_points", map12.get("send_points"));
