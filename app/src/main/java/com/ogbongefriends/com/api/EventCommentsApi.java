...
public class EventCommentsApi extends BasicApi implements Runnable {
	...
	public EventCommentsApi(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		//setPostData(map);
		url = Utils.getCompleteApiUrl(ctx, R.string.getEventComments);
					...
					try {
						...
						for(int i=0;i<userdata.size();i++){
							...
							JsonObject singleCountry=userdata.get(i).getAsJsonObject();	
								...
								db.autoInsertUpdate(DB.Table.Name.event_comment, data_to_event, DB.Table.event_comment.id+" = "+"'"+ singleCountry.get("id").getAsString() +"'", null);
							...
							db.autoInsertUpdate(DB.Table.Name.event_comment, data_to_event, DB.Table.event_comment.id+" = "+"'"+ temp_data.get("id").getAsString() +"'", null);
						}
						}
							else{
								...
								db.autoInsertUpdate(DB.Table.Name.event_comment, data_to_event, DB.Table.event_comment.id+" = "+"'"+ userdata.get("id").getAsString() +"'", null);
