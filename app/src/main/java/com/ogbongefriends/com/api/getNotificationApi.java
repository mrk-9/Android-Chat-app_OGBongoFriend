...
public class getNotificationApi extends BasicApi implements Runnable {
...
//	private SharedPreferences pref;
	...
	public getNotificationApi(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		...
		try {
			...
			if (obj == null) {
				...
				if (objJson.get("Data") != null) {
					...
					data.remove(DB.Table.user_notification_master.id.toString());
					data.put("uuid", pref.get(Constants.KeyUUID));
						db.open();
						db.autoInsertUpdate(DB.Table.Name.user_notification_master, data, DB.Table.user_notification_master.uuid+" = "+"'"+ pref.get(Constants.KeyUUID) +"'", null);
				db.close();
					}
				catch(Exception e)
				{
