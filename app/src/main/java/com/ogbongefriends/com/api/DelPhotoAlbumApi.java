...
public class DelPhotoAlbumApi extends BasicApi implements Runnable {
	...
	public DelPhotoAlbumApi(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		//setPostData(map);
		url = Utils.getCompleteApiUrl(ctx, R.string.delPhoto);
