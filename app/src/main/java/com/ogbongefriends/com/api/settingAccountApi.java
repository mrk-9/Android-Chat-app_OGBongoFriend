...
public class settingAccountApi extends BasicApi implements Runnable {
...
//	private SharedPreferences pref;
	...
	public settingAccountApi(Context ctx, DB db, CustomLoader p) {
		super(ctx, db);
		this.db = db;
		this.ctx = ctx;
		this.p = p;
		// url = "http://tmwtg.com/tmwtg/Cpanels/signup";
		url = Utils.getCompleteApiUrl(ctx, R.string.getSetting);
...
//				pref.commit();
					...
					JsonObject settingData=	objJson.get("Data").getAsJsonObject();
				HashMap<String, String>inserData=new HashMap<String, String>();
				inserData=CelUtils.getMapFromJsonObject(settingData);
				db.autoInsertUpdate(Table.Name.setting_master, inserData, Table.setting_master.id.toString()+" = '"+settingData.get("id").getAsString()+"'", null);
				db.close();
				}
